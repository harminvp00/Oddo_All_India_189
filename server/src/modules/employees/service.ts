...
function formatEmployee(emp: any) {
  ...
  return {
    ...
    hireDate: formatDateString(emp.hire_date),
    ...
    workingSchedule: emp.working_schedules
      ? {
    ...
    contractCount: emp._count?.contracts ?? 0,
    attendanceCount: emp._count?.attendance ?? 0,
    leaveRequestCount: emp._count?.leave_requests ?? 0,
    leaveAllocationCount: emp._count?.leave_allocations ?? 0,
...
export class EmployeeService {
  ...
  static async createEmployee(input: CreateEmployeeInput) {
    // 1. Uniqueness check for employeeCode
    const existingCode = await EmployeeRepository.findByEmployeeCode(input.employeeCode);
    if (existingCode) {
      const error = new Error(`Employee with code ${input.employeeCode} already exists`);
      (error as any).code = 'DUPLICATE_RESOURCE';
      throw error;
    }

    // 2. Uniqueness check for userId
    if (input.userId) {
      const existingUserLink = await EmployeeRepository.findByUserId(BigInt(input.userId));
      if (existingUserLink) {
    ...
    // 3. Validate referenced foreign keys
  ...
  static async updateEmployee(id: bigint, input: UpdateEmployeeInput) {
    ...
    if (input.userId !== undefined) updateData.user_id = input.userId ? BigInt(input.userId) : null;
  ...
  static async deleteEmployee(id: bigint) {
    ...
    if (hasRelations) {
      ...
      return {
        ...
        action: 'DEACTIVATED',
        message: 'Employee has related historical records (contracts/attendance/leaves) and was terminated instead of deleted.',
      };
    }
